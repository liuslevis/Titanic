...
formula <- Survived ~ Pclass + Sex + AgeD + SibSp + Parch + Fare + Embarked + Title + CabinD
...
ct <- cforest(formula=formula, data=train, control=ctree_control())
# ?ctree_control
# plot(ct)
# summary(combi)
# summary(train)
...
write.csv(submission, file="data/submit_cforest.csv", row.names=FALSE, quote=FALSE)
